...
void kernel_gpu_teams_parallel_data_omp(double T[][GRIDY + 2], double T_new[][GRIDY + 2],
                    int max_iterations, double dt) {
  int iteration = 0;

#pragma omp target enter data \
            map(to: T[:GRIDX + 2][:GRIDY + 2])   \
...
int main(int argc, char *argv[])
{
  ...
  kernel_gpu_teams_parallel_data_omp(T, T_new, max_iterations, dt);
