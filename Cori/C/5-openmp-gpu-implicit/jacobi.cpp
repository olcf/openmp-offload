...
// Pass T and T_new by reference
void init(double (& T )[GRIDX + 2][GRIDY + 2], 
          double (& T_new)[GRIDY + 2][GRIDY + 2]) {
...
void kernel_gpu_teams_parallel_implicit_omp(
...
#pragma omp target \
        teams distribute parallel for simd collapse(2) reduction(max : dt)
    for (int i = 1; i <= GRIDX; i++) {
      for (int j = 1; j <= GRIDY; j++) {
...
int main(int argc, char *argv[])
{
  ...
  printf("Running GPU Teams Parallel data OpenMP kernel\n\n");

  init(T, T_new);
  double start = omp_get_wtime();
  kernel_gpu_teams_parallel_implicit_omp(T, T_new, max_iterations, dt);
  double end = omp_get_wtime();
  printf("GPU Teams Parallel data OpenMP kernel time = %.6f Seconds\n\n", end - start);
